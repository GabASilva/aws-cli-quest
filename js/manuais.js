...
SERVIÇOS DISPONÍVEIS
    ...
    cloudtrail  Auditoria: quem fez o quê na conta
    ssm         Systems Manager (Parameter Store)
    cognito-idp Login de usuários (user pools)
    autoscaling Elasticidade automática
...
quando dá certo (igual ao AWS de verdade).`,
  ...
  "ec2.delete-vpc": `aws ec2 delete-vpc\n\nUSO\n    aws ec2 delete-vpc --vpc-id <vpc-id>\n\nApaga uma VPC. Ela só sai VAZIA — a AWS não apaga em cascata.\n\nA ORDEM DE DESMONTAR\n    1. aws ec2 delete-subnet --subnet-id <subnet-id>          (cada sub-rede)\n    2. aws ec2 detach-internet-gateway --internet-gateway-id <igw-id> --vpc-id <vpc-id>\n    3. aws ec2 delete-internet-gateway --internet-gateway-id <igw-id>\n    4. aws ec2 delete-vpc --vpc-id <vpc-id>\n\nFora de ordem: DependencyViolation, dizendo o que ainda está pendurado.`,
  "ec2.delete-subnet": `aws ec2 delete-subnet\n\nUSO\n    aws ec2 delete-subnet --subnet-id <subnet-id>\n\nApaga uma sub-rede. É o PRIMEIRO passo pra desmontar uma VPC: enquanto\nhouver sub-rede, o delete-vpc recusa.\n\nSub-rede com instância dentro também recusa (DependencyViolation).`,
  "ec2.detach-internet-gateway": `aws ec2 detach-internet-gateway\n\nUSO\n    aws ec2 detach-internet-gateway --internet-gateway-id <igw-id> --vpc-id <vpc-id>\n\nDesconecta o internet gateway da VPC. A partir daí nada sai pra\ninternet: as rotas que apontavam pro gateway viram "blackhole".\n\nPede as DUAS pontas (gateway e VPC). Se o gateway não estiver ligado\nnaquela VPC, o erro é Gateway.NotAttached.`,
  "ec2.delete-internet-gateway": `aws ec2 delete-internet-gateway\n\nUSO\n    aws ec2 delete-internet-gateway --internet-gateway-id <igw-id>\n\nApaga o internet gateway. Só funciona DEPOIS do detach-internet-gateway:\nconectado, dá DependencyViolation.`,
  ...
  cloudtrail: `aws cloudtrail — auditoria da conta\n\nA câmera de segurança da AWS: registra QUEM fez O QUÊ e QUANDO. É a\nprimeira coisa que se olha quando algo estranho acontece na conta.\n\nCOMANDOS\n    create-trail       cria a trilha (precisa de um bucket S3)\n    describe-trails    lista as trilhas\n    start-logging      COMEÇA a gravar (não é automático!)\n    stop-logging       para de gravar\n    get-trail-status   está gravando mesmo?\n    lookup-events      consulta os eventos registrados\n    delete-trail       apaga a trilha\n\nPEGADINHA CLÁSSICA: criar a trilha não liga a gravação. Sem o\nstart-logging, você acha que está auditado e não está.`,
  "cloudtrail.create-trail": `aws cloudtrail create-trail\n\nUSO\n    aws cloudtrail create-trail --name trilha-auditoria --s3-bucket-name meu-bucket-logs\n    aws cloudtrail create-trail --name trilha-auditoria --s3-bucket-name meu-bucket-logs --is-multi-region-trail\n\nO bucket precisa existir antes. Com --is-multi-region-trail a trilha\ncaptura eventos de TODAS as regiões (recomendado).`,
  "cloudtrail.describe-trails": `aws cloudtrail describe-trails\n\nUSO\n    aws cloudtrail describe-trails\n\nLista as trilhas da conta e pra qual bucket cada uma entrega.`,
  "cloudtrail.start-logging": `aws cloudtrail start-logging\n\nUSO\n    aws cloudtrail start-logging --name trilha-auditoria\n\nLiga a gravação. Sem isso a trilha existe mas não registra nada.`,
  "cloudtrail.stop-logging": `aws cloudtrail stop-logging\n\nUSO\n    aws cloudtrail stop-logging --name trilha-auditoria\n\nPausa a gravação (a trilha continua existindo).`,
  "cloudtrail.get-trail-status": `aws cloudtrail get-trail-status\n\nUSO\n    aws cloudtrail get-trail-status --name trilha-auditoria\n\nO campo IsLogging diz se está gravando de verdade.`,
  "cloudtrail.lookup-events": `aws cloudtrail lookup-events\n\nUSO\n    aws cloudtrail lookup-events\n    aws cloudtrail lookup-events --max-results 5\n    aws cloudtrail lookup-events --lookup-attributes AttributeKey=EventName,AttributeValue=RunInstances\n\nConsulta os últimos 90 dias de eventos. Filtros comuns:\n    AttributeKey=EventName    o nome da operação (RunInstances, CreateBucket)\n    AttributeKey=EventSource  o serviço (ec2.amazonaws.com)\n\nATENÇÃO: o CloudTrail grava o nome da API, não o comando que você\ndigitou. Os atalhos do 'aws s3' viram o nome real por baixo:\n    aws s3 mb  ->  CreateBucket        aws s3 cp  ->  PutObject\n    aws s3 rb  ->  DeleteBucket        aws s3 rm  ->  DeleteObject`,
  "cloudtrail.delete-trail": `aws cloudtrail delete-trail\n\nUSO\n    aws cloudtrail delete-trail --name trilha-auditoria\n\nApaga a trilha. Os logs já entregues no S3 continuam lá.`,

  // ===== Systems Manager / Parameter Store =====
  ssm: `aws ssm — Systems Manager (Parameter Store)\n\nGuarda CONFIGURAÇÃO fora do código: URLs, nomes de bucket, senhas.\nTrocar o valor não exige novo deploy — a aplicação lê na hora de subir.\n\nCOMANDOS\n    put-parameter           cria/atualiza um parâmetro\n    get-parameter           lê um parâmetro\n    get-parameters-by-path  lê TODOS de um caminho (/loja/)\n    describe-parameters     lista os parâmetros\n    delete-parameter        apaga\n\nTIPOS\n    String        texto simples\n    StringList    lista separada por vírgula\n    SecureString  cifrado com KMS (precisa de --with-decryption pra ler)\n\nDICA: use nomes hierárquicos (/loja/prod/url-api) — aí dá pra puxar\ntoda a config de um ambiente numa chamada só.\n\nSSM x SECRETS MANAGER: o Parameter Store é grátis e simples; o Secrets\nManager custa, mas faz rotação automática de senha.`,
  "ssm.put-parameter": `aws ssm put-parameter\n\nUSO\n    aws ssm put-parameter --name /loja/url-api --value https://api.loja.com --type String\n    aws ssm put-parameter --name /loja/senha-db --value s3nh4 --type SecureString\n    aws ssm put-parameter --name /loja/url-api --value nova --type String --overwrite\n\nSem --overwrite, atualizar um parâmetro existente dá erro.`,
  "ssm.get-parameter": `aws ssm get-parameter\n\nUSO\n    aws ssm get-parameter --name /loja/url-api\n    aws ssm get-parameter --name /loja/senha-db --with-decryption\n\nSecureString só mostra o valor real com --with-decryption (e se você\ntiver permissão na chave KMS).`,
  "ssm.get-parameters-by-path": `aws ssm get-parameters-by-path\n\nUSO\n    aws ssm get-parameters-by-path --path /loja/\n    aws ssm get-parameters-by-path --path /loja/ --with-decryption\n\nTraz todos os parâmetros daquele caminho — é assim que uma aplicação\ncarrega a configuração inteira ao iniciar.`,
  "ssm.describe-parameters": `aws ssm describe-parameters\n\nUSO\n    aws ssm describe-parameters\n\nLista os parâmetros (nome, tipo e versão) — sem os valores.`,
  "ssm.delete-parameter": `aws ssm delete-parameter\n\nUSO\n    aws ssm delete-parameter --name /loja/url-api\n\nApaga o parâmetro (não tem janela de recuperação como no Secrets Manager).`,

  // ===== Cognito =====
  "cognito-idp": `aws cognito-idp — login de usuários (user pools)\n\nCadastro, login, confirmação por e-mail, recuperação de senha e MFA\nprontos — você não escreve autenticação nem guarda senha.\n\nAS DUAS PEÇAS\n    USER POOL   o banco de usuários (quem pode entrar)\n    CLIENT      o app que usa o pool (site, celular) — tem um ClientId\n\nCOMANDOS\n    create-user-pool / list-user-pools / describe-user-pool / delete-user-pool\n    create-user-pool-client\n    admin-create-user / list-users / admin-delete-user\n\nOs comandos "admin-" são os que VOCÊ (dono) roda; existe outro conjunto\n(sign-up, initial-auth) que o app do usuário final usa.`,
  "cognito-idp.create-user-pool": `aws cognito-idp create-user-pool\n\nUSO\n    aws cognito-idp create-user-pool --pool-name usuarios-loja\n\nCria o banco de usuários. Guarde o Id (formato us-east-1_ABC123XYZ).`,
  "cognito-idp.list-user-pools": `aws cognito-idp list-user-pools\n\nUSO\n    aws cognito-idp list-user-pools --max-results 10\n\nATENÇÃO: --max-results é OBRIGATÓRIO neste comando (esquisito, mas é\nassim na AWS de verdade).`,
  "cognito-idp.describe-user-pool": `aws cognito-idp describe-user-pool\n\nUSO\n    aws cognito-idp describe-user-pool --user-pool-id us-east-1_ABC123\n\nMostra a configuração do pool, incluindo a política de senha.`,
  "cognito-idp.create-user-pool-client": `aws cognito-idp create-user-pool-client\n\nUSO\n    aws cognito-idp create-user-pool-client --user-pool-id <id> --client-name app-web\n\nCria o "app client". O ClientId que volta é o que vai no código do\nfront-end pra falar com o Cognito.`,
  "cognito-idp.admin-create-user": `aws cognito-idp admin-create-user\n\nUSO\n    aws cognito-idp admin-create-user --user-pool-id <id> --username maria\n\nCria o usuário como administrador. Ele nasce em FORCE_CHANGE_PASSWORD:\nprecisa trocar a senha temporária no primeiro login.`,
  "cognito-idp.list-users": `aws cognito-idp list-users\n\nUSO\n    aws cognito-idp list-users --user-pool-id <id>\n\nLista os usuários e o UserStatus de cada um (CONFIRMED,\nUNCONFIRMED, FORCE_CHANGE_PASSWORD...).`,
  "cognito-idp.admin-delete-user": `aws cognito-idp admin-delete-user\n\nUSO\n    aws cognito-idp admin-delete-user --user-pool-id <id> --username maria\n\nRemove o usuário do pool.`,
  "cognito-idp.delete-user-pool": `aws cognito-idp delete-user-pool\n\nUSO\n    aws cognito-idp delete-user-pool --user-pool-id <id>\n\nApaga o pool e TODOS os usuários dentro dele. Não tem volta.`,

  // ===== Auto Scaling =====
  autoscaling: `aws autoscaling — elasticidade automática\n\nMantém N máquinas no ar sozinho: se uma cai, sobe outra; se o tráfego\naumenta, adiciona; se diminui, remove (e para de custar).\n\nOS TRÊS NÚMEROS\n    min-size          o mínimo que sempre fica no ar\n    max-size          o teto (proteção contra susto na fatura)\n    desired-capacity  quantas você quer AGORA\n\nCOMANDOS\n    create-auto-scaling-group / describe-auto-scaling-groups\n    set-desired-capacity        muda a capacidade na mão\n    update-auto-scaling-group   muda min/max/desired\n    put-scaling-policy          faz escalar SOZINHO (por CPU, etc.)\n    delete-auto-scaling-group\n\nA receita da máquina vem do LAUNCH TEMPLATE:\n    aws ec2 create-launch-template ...`,
  "autoscaling.create-auto-scaling-group": `aws autoscaling create-auto-scaling-group\n\nUSO\n    aws autoscaling create-auto-scaling-group --auto-scaling-group-name grupo-web \\\n        --launch-template LaunchTemplateName=modelo-web,Version=1 \\\n        --min-size 1 --max-size 4 --desired-capacity 2 \\\n        --availability-zones us-east-1a\n\nAssim que criado, ele já sobe as instâncias — confira com\n'aws ec2 describe-instances'.`,
  "autoscaling.describe-auto-scaling-groups": `aws autoscaling describe-auto-scaling-groups\n\nUSO\n    aws autoscaling describe-auto-scaling-groups\n\nMostra min/max/desejado e quais instâncias estão no grupo.`,
  "autoscaling.set-desired-capacity": `aws autoscaling set-desired-capacity\n\nUSO\n    aws autoscaling set-desired-capacity --auto-scaling-group-name grupo-web --desired-capacity 4\n\nSobe ou desce a quantidade agora. Precisa ficar entre min e max.`,
  "autoscaling.update-auto-scaling-group": `aws autoscaling update-auto-scaling-group\n\nUSO\n    aws autoscaling update-auto-scaling-group --auto-scaling-group-name grupo-web --min-size 0 --desired-capacity 0\n\nMuda os limites do grupo. É o que se usa pra "zerar" antes de apagar.`,
  "autoscaling.put-scaling-policy": `aws autoscaling put-scaling-policy\n\nUSO\n    aws autoscaling put-scaling-policy --auto-scaling-group-name grupo-web --policy-name cpu-alvo --policy-type TargetTrackingScaling\n\nCom a política o grupo escala SOZINHO conforme a métrica (ex.: manter\na CPU média em 60%). É o que torna o Auto Scaling automático de fato.`,
  "autoscaling.delete-auto-scaling-group": `aws autoscaling delete-auto-scaling-group\n\nUSO\n    aws autoscaling delete-auto-scaling-group --auto-scaling-group-name grupo-web\n    aws autoscaling delete-auto-scaling-group --auto-scaling-group-name grupo-web --force-delete\n\nCom instâncias no ar a AWS recusa: zere antes (--min-size 0\n--desired-capacity 0) ou use --force-delete.`,

  // ===== Launch template (dentro do aws ec2) =====
  "ec2.create-launch-template": `aws ec2 create-launch-template\n\nUSO\n    aws ec2 create-launch-template --launch-template-name modelo-web \\\n        --launch-template-data '{"ImageId":"ami-0abcd1234ef567890","InstanceType":"t2.micro"}'\n\nA "receita" da máquina: imagem, tipo, chave, security group. O Auto\nScaling usa ela pra subir cópias idênticas.`,
  "ec2.describe-launch-templates": `aws ec2 describe-launch-templates\n\nUSO\n    aws ec2 describe-launch-templates\n\nLista os launch templates da conta.`,

  // ===== Rede a fundo: route tables, network ACLs, flow logs =====
  "ec2.create-route-table": `aws ec2 create-route-table\n\nUSO\n    aws ec2 create-route-table --vpc-id vpc-xxxx\n\nCria uma tabela de rotas. Ela já nasce com a rota "local" (o tráfego\ndentro da própria VPC), e você adiciona as outras com create-route.`,
  "ec2.describe-route-tables": `aws ec2 describe-route-tables\n\nUSO\n    aws ec2 describe-route-tables\n    aws ec2 describe-route-tables --route-table-ids rtb-xxxx\n    aws ec2 describe-route-tables --filter "Name=association.subnet-id,Values='subnet-xxxx'"\n\nO QUE OLHAR: uma sub-rede é PÚBLICA quando a tabela dela tem uma rota\n0.0.0.0/0 apontando pra um internet gateway (igw-). Sem essa rota, a\nsub-rede é privada — mesmo que o nome dela diga "public".`,
  "ec2.create-route": `aws ec2 create-route\n\nUSO\n    aws ec2 create-route --route-table-id rtb-xxxx --gateway-id igw-xxxx --destination-cidr-block 0.0.0.0/0\n\nEssa é a rota que dá saída pra internet: "tudo que não for da própria\nVPC, manda pro internet gateway".`,
  "ec2.delete-route": `aws ec2 delete-route\n\nUSO\n    aws ec2 delete-route --route-table-id rtb-xxxx --destination-cidr-block 0.0.0.0/0\n\nRemove uma rota. A rota "local" não pode ser removida.`,
  "ec2.associate-route-table": `aws ec2 associate-route-table\n\nUSO\n    aws ec2 associate-route-table --route-table-id rtb-xxxx --subnet-id subnet-xxxx\n\nLiga a tabela a uma sub-rede. Sem associação explícita, a sub-rede usa\na tabela PRINCIPAL (main) da VPC.`,
  "ec2.describe-internet-gateways": `aws ec2 describe-internet-gateways\n\nUSO\n    aws ec2 describe-internet-gateways\n\nLista os gateways e a qual VPC cada um está conectado (Attachments).`,
  "ec2.describe-network-acls": `aws ec2 describe-network-acls\n\nUSO\n    aws ec2 describe-network-acls\n    aws ec2 describe-network-acls --filter "Name=association.subnet-id,Values='subnet-xxxx'"\n\nA network ACL é o firewall da SUB-REDE (o security group é o da\ninstância). Duas diferenças que derrubam muita gente:\n    - vale a regra de MENOR número que casar (uma deny 40 vence uma\n      allow 100);\n    - ela NÃO tem estado: precisa liberar entrada E saída.`,
  "ec2.create-network-acl-entry": `aws ec2 create-network-acl-entry\n\nUSO\n    aws ec2 create-network-acl-entry --network-acl-id acl-xxxx --rule-number 100 \\\n        --protocol tcp --port-range From=80,To=80 --cidr-block 0.0.0.0/0 \\\n        --rule-action allow --ingress\n\n--ingress = entrada, --egress = saída. Escolha o número pensando na\nordem: menor número decide primeiro.`,
  "ec2.delete-network-acl-entry": `aws ec2 delete-network-acl-entry\n\nUSO\n    aws ec2 delete-network-acl-entry --network-acl-id acl-xxxx --ingress --rule-number 40\n\nApaga uma regra. Precisa dizer se é --ingress ou --egress (podem\nexistir duas regras com o mesmo número, uma de cada lado).`,
  "ec2.create-flow-logs": `aws ec2 create-flow-logs\n\nUSO\n    aws ec2 create-flow-logs --resource-type VPC --resource-ids vpc-xxxx \\\n        --traffic-type ALL --log-destination-type s3 \\\n        --log-destination arn:aws:s3:::meu-bucket-de-logs\n\nRegistra o tráfego que passa pelas interfaces de rede. traffic-type:\nALL, ACCEPT ou REJECT. O bucket precisa existir antes.\n\nCADA LINHA tem: version account-id interface-id srcaddr dstaddr\nsrcport dstport protocol packets bytes start end action log-status\n(o 7º campo é a porta de destino; o penúltimo é ACCEPT ou REJECT).`,
  "ec2.describe-flow-logs": `aws ec2 describe-flow-logs\n\nUSO\n    aws ec2 describe-flow-logs\n\nMostra os flow logs e o FlowLogStatus (precisa estar ACTIVE).`,
  "ec2.delete-flow-logs": `aws ec2 delete-flow-logs\n\nUSO\n    aws ec2 delete-flow-logs --flow-log-ids fl-xxxx\n\nPara de registrar. Os logs já entregues no S3 continuam lá.`,
  "ec2.describe-network-interfaces": `aws ec2 describe-network-interfaces\n\nUSO\n    aws ec2 describe-network-interfaces\n    aws ec2 describe-network-interfaces --filters "Name=association.public-ip,Values='198.51.100.42'"\n\nA interface de rede (eni-...) é o "cabo" da instância. O id dela é o\nque aparece nos flow logs — é assim que você liga uma linha de log à\nmáquina que recebeu a conexão.`,
};
