...
(function () {
  ...
  Object.assign(SERVICOS.s3api, {
    ...
    "put-bucket-tagging": (conta, pos, flags) => {
      ...
      // SUBSTITUI o conjunto inteiro, como na AWS (o manual diz isso). A versão
      // de 22/09 somava — e ensinava errado exatamente a pegadinha do comando.
      b.tags = {};
  ...
  // Revisão de 24/09: uma atividade por comando novo, e fixação em cenário
  // novo pra cada família. O get-bucket-lifecycle-configuration, que tinha
  // manual e nenhuma atividade, entra aqui.
  at("s3-13", [
    d("s3c-mv1", "s3", 2, 80, "O arquivo subiu com o nome errado",
      ...
      ["O comando tem duas letras e é o mesmo do Linux.", "Origem e destino são os dois caminhos `s3://`, completos."],
    ...
    d("s3c-presign1", "s3", 2, 90, "O cliente quer o PDF, mas o bucket é privado",
      ...
      ["Existe um comando do `aws s3` que assina uma URL temporária pro objeto.", "O prazo vai em `--expires-in`, em segundos (o máximo da AWS são 7 dias).", "Repare: quem tiver o link acessa — o link É o segredo."],
    ...
    d("s3c-fix-alto", "s3", 2, 90, "O contrato final, com nome certo e link de uma hora",
      "O rascunho do contrato subiu como <b>rascunho.csv</b> no <b>contratos-2026</b>. Suba, renomeie pra <b>contrato-final.csv</b> e mande pro cliente um link que vale <b>3600</b> segundos.",
      ["Subir é o cp de sempre, já com o nome de destino.", "Renomear é o mv; o link, o presign — os dois das atividades anteriores."],
      ["aws s3 cp relatorio.csv s3://contratos-2026/rascunho.csv",
        "aws s3 mv s3://contratos-2026/rascunho.csv s3://contratos-2026/contrato-final.csv",
        "aws s3 presign s3://contratos-2026/contrato-final.csv --expires-in 3600"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3", "presign") && !!obj(c, "contratos-2026", "contrato-final.csv")),
  ]);

  at("cob-s3-3", [
    d("s3c-block1", "s3", 3, 130, "As quatro travas que evitam a manchete",
      "Todo vazamento de S3 que virou notícia tem a mesma origem: bucket que ficou público sem ninguém perceber. Ligue as <b>quatro travas</b> no <b>contratos-2026</b>.",
      ["São quatro chaves numa flag só, separadas por vírgula — e as quatro começam com Block, Ignore ou Restrict.", "Com as quatro ligadas, nem ACL nem política conseguem expor o bucket, mesmo que alguém tente."],
    ...
    d("s3c-block1b", "s3", 3, 110, "Confira as travas do bucket fiscal",
      "O <b>docs-fiscais</b> guarda nota fiscal e não pode vazar. Ligue as quatro travas nele também e <b>confira</b> que ficaram valendo.",
      ["A gravação é a mesma da atividade anterior.", "Pra conferir existe o `get-…` correspondente, só com o --bucket."],
      ["aws s3api put-public-access-block --bucket docs-fiscais --public-access-block-configuration BlockPublicAcls=true,IgnorePublicAcls=true,BlockPublicPolicy=true,RestrictPublicBuckets=true",
        "aws s3api get-public-access-block --bucket docs-fiscais"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3api", "get-public-access-block") && !!((bkt(c, "docs-fiscais") || {}).bloqueio)),
    d("s3c-enc1", "s3", 3, 120, "A auditoria exige tudo cifrado",
      ...
      ["A configuração vai em JSON na flag `--server-side-encryption-configuration`.", "A estrutura tem Rules ➜ ApplyServerSideEncryptionByDefault ➜ SSEAlgorithm.", "AES256 é a chave da AWS; `aws:kms` seria uma chave sua, auditável e cobrada por chamada."],
    ...
    d("s3c-enc1b", "s3", 3, 120, "Nota fiscal com chave própria",
      "Pra nota fiscal o jurídico quer poder auditar quem decifrou o quê — isso pede chave do KMS, não a da AWS. Ligue a criptografia do <b>docs-fiscais</b> com <b>aws:kms</b> e <b>confira</b> o que ficou valendo.",
      ["É o mesmo comando, trocando o SSEAlgorithm pra `aws:kms`.", "Pra conferir existe o `get-bucket-encryption`."],
      ["aws s3api put-bucket-encryption --bucket docs-fiscais --server-side-encryption-configuration '{\"Rules\":[{\"ApplyServerSideEncryptionByDefault\":{\"SSEAlgorithm\":\"aws:kms\"}}]}'",
        "aws s3api get-bucket-encryption --bucket docs-fiscais"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3api", "get-bucket-encryption") &&
        (((bkt(c, "docs-fiscais") || {}).criptografia) || {}).algoritmo === "aws:kms"),
    d("s3c-life1", "s3", 3, 150, "Log de 2019 na classe mais cara",
      ...
      ["Ciclo de vida é uma REGRA que a AWS aplica sozinha todo dia — você não roda nada depois.", "O documento vai em JSON, com Rules ➜ Transitions ➜ Days e StorageClass.", "O Filter ➜ Prefix é o que limita a regra a uma pasta em vez do bucket inteiro."],
    ...
    d("s3c-life1b", "s3", 3, 120, "A nota fiscal desce depois de 60 dias",
      "Nota fiscal é consultada no primeiro mês e quase nunca depois. Configure o <b>docs-fiscais</b> pra descer pra <b>STANDARD_IA</b> depois de <b>60</b> dias — e <b>leia a regra de volta</b> pra conferir.",
      ["A gravação é o mesmo `put-bucket-lifecycle-configuration`, com outra classe e outro prazo.", "Ler a regra é o `get-bucket-lifecycle-configuration`, só com o --bucket."],
      ["aws s3api put-bucket-lifecycle-configuration --bucket docs-fiscais --lifecycle-configuration '{\"Rules\":[{\"ID\":\"esfriar-notas\",\"Status\":\"Enabled\",\"Filter\":{\"Prefix\":\"notas/\"},\"Transitions\":[{\"Days\":60,\"StorageClass\":\"STANDARD_IA\"}]}]}'",
        "aws s3api get-bucket-lifecycle-configuration --bucket docs-fiscais"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3api", "get-bucket-lifecycle-configuration") &&
        (((bkt(c, "docs-fiscais") || {}).cicloVida) || {}).classe === "STANDARD_IA"),
    d("s3c-ver1", "s3", 3, 120, "Apague num bucket versionado",
      "Monte o bucket <b>arquivos-versionados</b> com versionamento ligado, suba o <b>relatorio.csv</b> e <b>apague</b> ele usando o apagar em lote do s3api.",
      ["Primeiro ligue o versionamento com o put-bucket-versioning (Status=Enabled).", "Apagar em lote é `delete-objects`, com a lista em `Objects=[{Key=...}]`."],
      ...
      (c) => !!(((bkt(c, "arquivos-versionados") || {}).versoes || {})["relatorio.csv"])),
    d("s3c-ver1b", "s3", 3, 120, "Apagar com versionamento não apaga",
      "O arquivo sumiu da listagem normal. Agora <b>liste as versões</b> do <b>arquivos-versionados</b> e veja com os próprios olhos: o conteúdo continua lá, com uma lápide (DeleteMarker) por cima — e continua na fatura.",
      ["O `list-object-versions` é o único comando que mostra as lápides.", "Tirar a lápide é o jeito de desfazer um delete com versionamento ligado."],
      ["aws s3api list-object-versions --bucket arquivos-versionados"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3api", "list-object-versions")),
    d("s3c-fix-ver", "s3", 3, 130, "Dois arquivos apagados, nenhum perdido",
      "Crie <b>contratos-versionados</b> com versionamento, suba o <b>relatorio.csv</b> e o <b>logo.png</b>, apague os dois numa chamada só e confira nas versões que nenhum dos dois se perdeu de verdade.",
      ["Tudo aqui você já fez: versionamento, cp, delete-objects e list-object-versions.", "No lote, cada arquivo é um `{Key=...}` dentro de `Objects=[...]`."],
      ["aws s3 mb s3://contratos-versionados",
        "aws s3api put-bucket-versioning --bucket contratos-versionados --versioning-configuration Status=Enabled",
        "aws s3 cp relatorio.csv s3://contratos-versionados/",
        "aws s3 cp logo.png s3://contratos-versionados/",
        "aws s3api delete-objects --bucket contratos-versionados --delete 'Objects=[{Key=relatorio.csv},{Key=logo.png}]'",
        "aws s3api list-object-versions --bucket contratos-versionados"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3api", "list-object-versions") &&
        Object.keys(((bkt(c, "contratos-versionados") || {}).versoes) || {}).length >= 2),
    d("s3c-head1", "s3", 3, 100, "Qual o tamanho daquele arquivo de 4 GB?",
      "Você só precisa saber o tamanho e a data — baixar o arquivo inteiro pra descobrir seria absurdo (e você paga a transferência). Confira o <b>relatorio.csv</b> do <b>contratos-2026</b> sem baixar.",
      ["Existe uma família de comandos que pergunta o cabeçalho sem trazer o corpo.", "Pro objeto, ela pede o bucket e a chave."],
      ["aws s3api head-object --bucket contratos-2026 --key relatorio.csv"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3api", "head-object")),
    d("s3c-head1b", "s3", 3, 90, "O script tem acesso ao bucket?",
      "Antes de o script de backup rodar à meia-noite, teste do jeito mais barato possível se o bucket <b>contratos-2026</b> existe e se você tem acesso.",
      ["É o irmão do comando anterior, pro bucket.", "Resposta VAZIA é sucesso — é o teste de permissão mais barato que existe."],
    ...
    d("s3c-copy1", "s3", 3, 120, "Copie 4 GB sem baixar 4 GB",
      ...
      ["O `s3api copy-object` manda a AWS copiar internamente: nada desce e nada sobe.", "Repare no formato do `--copy-source`: é `<bucket>/<chave>`, SEM o s3:// na frente.", "O `--bucket` e o `--key` aqui são o DESTINO."],
    ...
    d("s3c-tag1", "s3", 3, 100, "Quanto esse bucket custa pro time?",
      "O relatório de custo mostra o S3 num bolo só. Etiquete o <b>logs-aplicacao</b> com <b>Time=plataforma</b>.",
      ["A tag vai numa estrutura chamada TagSet, em forma de lista.", "A forma é `--tagging 'TagSet=[{Key=<chave>,Value=<valor>}]'`."],
    ...
    d("s3c-tag1b", "s3", 3, 110, "O put que apagou a etiqueta do outro time",
      "O financeiro pediu o centro de custo <b>cc-2040</b> no <b>logs-aplicacao</b>. Cuidado: este comando <b>SUBSTITUI</b> o conjunto inteiro — gravar só a tag nova apagaria a <b>Time</b>. Grave as duas juntas e leia de volta.",
      ["Não existe 'adicionar uma tag' aqui: o TagSet que você manda vira o conjunto inteiro.", "As duas vão na mesma lista: `TagSet=[{Key=Time,Value=plataforma},{Key=Custo,Value=cc-2040}]`.", "Ler é o `get-bucket-tagging`."],
      ["aws s3api put-bucket-tagging --bucket logs-aplicacao --tagging 'TagSet=[{Key=Time,Value=plataforma},{Key=Custo,Value=cc-2040}]'",
        "aws s3api get-bucket-tagging --bucket logs-aplicacao"],
      (c, cmd, ok) => {
        const t = ((bkt(c, "logs-aplicacao") || {}).tags) || {};
        return ok && ehCmd(cmd, "s3api", "get-bucket-tagging") && t.Time === "plataforma" && t.Custo === "cc-2040";
      }),
    ...
    d("s3c-fix-rest", "s3", 3, 120, "O arquivo mais frio de todos",
      "A auditoria de 2023 foi guardada no <b>DEEP_ARCHIVE</b>, a classe mais barata e mais lenta. Copie o <b>relatorio.csv</b> pro <b>logs-aplicacao</b> como <b>antigo-2023.csv</b> nessa classe e peça a restauração por <b>3</b> dias.",
      ["É o mesmo par copy-object + restore-object, com a classe mais fria.", "No DEEP_ARCHIVE a restauração pode levar até 12 horas — por isso se pede com antecedência."],
      ["aws s3api copy-object --bucket logs-aplicacao --key antigo-2023.csv --copy-source contratos-2026/relatorio.csv --storage-class DEEP_ARCHIVE",
        "aws s3api restore-object --bucket logs-aplicacao --key antigo-2023.csv --restore-request Days=3"],
      (c) => ((obj(c, "logs-aplicacao", "antigo-2023.csv") || {}).restaurando) === 3),
    d("s3c-pol1", "s3", 3, 110, "Revogue a política que abria o bucket",
      "O site antigo saiu do ar e a política que liberava leitura pública continua valendo — permissão que sobra é incidente esperando. Recrie o cenário no bucket <b>site-aposentado</b>: ponha a política pública nele, <b>leia</b> o que está valendo e só então <b>remova</b>.",
      ["Primeiro olhe o que está valendo com o `get-bucket-policy`: apagar política sem ler é como apagar regra de firewall no escuro.", "O comando de remover é o `delete-…` do mesmo par.", "A política vem do arquivo pronto: `file://politica-publica.json` (digite `ls` pra ver)."],
    ...
    d("s3c-fix-pol", "s3", 3, 110, "A política de teste que ficou no bucket de logs",
      "Alguém testou uma política pública no <b>logs-aplicacao</b> e esqueceu. Reproduza (ponha a política), confira que ela está lá e remova.",
      ["Pôr, ler e remover: os três comandos de política de bucket.", "A política vem do mesmo `file://politica-publica.json`."],
      ["aws s3api put-bucket-policy --bucket logs-aplicacao --policy file://politica-publica.json",
        "aws s3api get-bucket-policy --bucket logs-aplicacao",
        "aws s3api delete-bucket-policy --bucket logs-aplicacao"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3api", "delete-bucket-policy") && !((bkt(c, "logs-aplicacao") || {}).politica)),
    d("s3c-fix-audit", "s3", 3, 150, "A auditoria de um bucket, do começo ao fim",
      "Chegou o checklist de auditoria do S3. Pra <b>contratos-2026</b>: existe e há acesso? em que região? as quatro travas estão ligadas? está cifrado? e o tamanho do <b>relatorio.csv</b>? Pra <b>logs-aplicacao</b>: quem é o dono (tags) e qual a regra de ciclo de vida?",
      ["Nenhum comando novo: é a família inteira de `head-` e `get-` desta trilha.", "O head-bucket responde vazio quando está tudo certo — isso é sucesso."],
      ["aws s3api head-bucket --bucket contratos-2026",
        "aws s3api get-bucket-location --bucket contratos-2026",
        "aws s3api get-public-access-block --bucket contratos-2026",
        "aws s3api get-bucket-encryption --bucket contratos-2026",
        "aws s3api head-object --bucket contratos-2026 --key relatorio.csv",
        "aws s3api get-bucket-tagging --bucket logs-aplicacao",
        "aws s3api get-bucket-lifecycle-configuration --bucket logs-aplicacao"],
      (c, cmd, ok) => ok && ehCmd(cmd, "s3api", "get-bucket-lifecycle-configuration") &&
        String(cmd.flags.bucket || "") === "logs-aplicacao"),
  ]);
})();
