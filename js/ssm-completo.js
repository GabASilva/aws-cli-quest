...
(function () {
  ...
  Object.assign(SERVICOS.ssm, {
    ...
    "list-documents": (conta, pos, flags) => {
      ...
      const tipo = /DocumentType/.test(filtro) ? filtro.match(/Values=([A-Za-z]+)/) : null;
  ...
  at("ssm-6", [
    ...
    // Fixação (revisão de 24/09): os dois comandos de leitura de parâmetro, de novo.
    d("ssmc-fix-param", "ssm", 3, 110, "O timeout do checkout mudou duas vezes",
      "O checkout começou a cancelar pedido e ninguém lembra quanto era o timeout. Crie <b>/loja/checkout-timeout</b> com <b>30</b>, mude pra <b>45</b>, veja o histórico — e no fim leia o timeout e a URL da API numa chamada só, como a aplicação faz no boot.",
      ["A segunda gravação precisa do `--overwrite`.", "O histórico é o `get-parameter-history`; a leitura de vários é o `get-parameters` com os nomes separados por espaço."],
      ["aws ssm put-parameter --name /loja/checkout-timeout --value 30 --type String",
        "aws ssm put-parameter --name /loja/checkout-timeout --value 45 --type String --overwrite",
        "aws ssm get-parameter-history --name /loja/checkout-timeout",
        "aws ssm get-parameters --names /loja/checkout-timeout /loja/url-api"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "get-parameters") &&
        ((param(c, "/loja/checkout-timeout") || {}).versao || 0) >= 2),
  ]);

  // --- operar a frota: Run Command, Session Manager e runbook ---
  // Revisão de 24/09: run1, run2 e sess2 introduziam dois comandos cada; agora
  // é um por atividade, e cada família fecha com uma fixação em cenário novo.
  at("cob-ssm-1", [
    ...
    d("ssmc-fix-inv", "ssm", 2, 80, "O checklist antes do plantão",
      "Começo de plantão: confira quais máquinas o SSM está enxergando e liste <b>só os runbooks</b> (documentos do tipo <b>Automation</b>) que você vai ter à mão se algo quebrar.",
      ["Os dois comandos das atividades anteriores, na ordem de um checklist.", "O filtro de tipo vai em `--filters Key=DocumentType,Values=Automation`."],
      ["aws ssm describe-instance-information",
        "aws ssm list-documents --filters Key=DocumentType,Values=Automation"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "list-documents") && String(cmd.flags.filters || "").indexOf("Automation") >= 0),
    d("ssmc-run0", "ssm", 2, 80, "A máquina que você vai operar sem SSH",
      "Pra praticar Run Command você precisa de uma máquina gerenciada. Suba uma instância <b>t3.micro</b> — é nela que você vai rodar comando, abrir sessão e disparar runbook nesta trilha, sem nunca abrir a porta 22.",
      ["É o mesmo `run-instances` da trilha de EC2.", "Precisa da imagem (`--image-id`) e do tipo (`--instance-type`)."],
      ["aws ec2 run-instances --image-id ami-0abcd1234ef567890 --instance-type t3.micro"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ec2", "run-instances") && String(cmd.flags["instance-type"] || "") === "t3.micro"),
    d("ssmc-run1", "ssm", 3, 120, "Rode um comando sem abrir SSH",
      "Precisa saber há quanto tempo o servidor está de pé, mas a porta 22 está fechada e você não tem a chave .pem. Rode <b>uptime</b> na instância pelo Run Command, com o documento <b>AWS-RunShellScript</b>.",
    ...
    d("ssmc-run2", "ssm", 3, 90, "O histórico do que foi rodado",
      "O send-command devolveu um protocolo, não a resposta. Antes de buscar a saída, veja o <b>histórico de envios</b> da conta — é lá que está o CommandId.",
      ["É um `list-…` dos comandos, sem argumento obrigatório.", "Repare nos campos: documento, quantas máquinas, quantas deram erro."],
      ["aws ssm list-commands"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "list-commands")),
    d("ssmc-run2b", "ssm", 3, 110, "E aí, deu certo?",
      "Com o CommandId na mão, busque a <b>saída</b> do uptime naquela máquina. <small>(é por isso que operar pela CLI dá certo em 300 máquinas: a saída de cada uma fica guardada)</small>",
      ["Você precisa de duas coisas: qual envio e qual máquina.", "A saída vem em StandardOutputContent."],
    ...
    d("ssmc-run3", "ssm", 3, 120, "Reinicie o nginx da frota",
      "O time relatou que o site voltou a responder depois de reiniciar o nginx na mão — e isso precisa virar rotina, não heroísmo. Rode <b>systemctl restart nginx</b> na instância, deixando o comentário <b>reinicio nginx chamado 4412</b>.",
      ["Mesmo send-command do exercício anterior, outro conteúdo em commands — e aqui o comando tem espaços, então vai entre aspas.", "O `--comment` não é enfeite: é ele que aparece no histórico dizendo POR QUE aquilo foi rodado.", "No fim, veja máquina por máquina como foi: `list-command-invocations --details`."],
    ...
    d("ssmc-fix-run", "ssm", 3, 130, "O disco está enchendo em algum lugar",
      "O alarme avisou de disco quase cheio, mas não disse em qual máquina. Rode <b>df -h</b> pelo Run Command com o comentário <b>checagem de disco</b>, ache o envio no histórico, leia a saída na instância e veja o resultado máquina por máquina.",
      ["Nenhum comando novo: send-command, list-commands, get-command-invocation e list-command-invocations.", "Comando e comentário com espaço vão entre aspas."],
      ["aws ssm send-command --instance-ids <id-da-instância> --document-name AWS-RunShellScript --parameters commands=\"df -h\" --comment \"checagem de disco\"",
        "aws ssm list-commands",
        "aws ssm get-command-invocation --command-id <comando-id> --instance-id <id-da-instância>",
        "aws ssm list-command-invocations --command-id <comando-id> --details"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "list-command-invocations") &&
        comandos(c).some((x) => String(x.comentario).indexOf("disco") >= 0)),
    ...
    d("ssmc-sess2", "ssm", 3, 90, "Quem está dentro das máquinas agora?",
      "Auditoria de segurança: liste as sessões <b>abertas neste momento</b>. <small>(sessão esquecida é terminal vivo numa máquina de produção)</small>",
      ["O `--state` é obrigatório e só aceita dois valores: Active ou History.", "Repare no Reason de cada sessão: é o rastro que o --reason deixou."],
      ["aws ssm describe-sessions --state Active"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "describe-sessions") && String(cmd.flags.state || "") === "Active"),
    d("ssmc-sess2b", "ssm", 3, 100, "Feche a sessão que você esqueceu aberta",
      "A sessão de investigar o disco ficou aberta. <b>Encerre</b> ela — ela não some, vai pro histórico.",
      ["Encerrar pede o SessionId que apareceu na listagem.", "É um `terminate-…`, e ele não apaga o registro de quem entrou."],
    ...
    d("ssmc-fix-sess", "ssm", 3, 120, "Auditoria trimestral de acesso",
      "Uma auditora vai entrar na máquina pra conferir configuração. Abra a sessão com o motivo <b>auditoria-trimestral</b>, confira que ela aparece entre as abertas, encerre no fim e mostre o <b>histórico</b> — que é o que a auditoria leva de prova.",
      ["Abrir, listar abertas, encerrar, listar histórico: tudo que você já fez nesta trilha.", "O histórico é o mesmo `describe-sessions`, com `--state History`."],
      ["aws ssm start-session --target <id-da-instância> --reason auditoria-trimestral",
        "aws ssm describe-sessions --state Active",
        "aws ssm terminate-session --session-id <sessao-id>",
        "aws ssm describe-sessions --state History"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "describe-sessions") && String(cmd.flags.state || "") === "History" &&
        sessoes(c).some((s) => String(s.motivo).indexOf("auditoria") >= 0 && s.estado === "Terminated")),
    ...
    d("ssmc-auto2", "ssm", 3, 100, "O runbook rodou mesmo?",
      "De manhã, o relatório: liste as automações executadas e veja como cada uma terminou.",
      ["É o `describe-…` das automações, sem argumento.", "O identificador de cada uma é o AutomationExecutionId, não o CommandId."],
      ["aws ssm describe-automation-executions"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "describe-automation-executions")),
    d("ssmc-auto2b", "ssm", 3, 110, "Em que passo o runbook parou?",
      "Abra a automação de reinício <b>passo a passo</b>. É no passo que falha — parar, ligar ou conferir — que mora a resposta de por que o runbook não resolveu.",
      ["O detalhe é da família `get-…` e pede o AutomationExecutionId.", "Repare em StepExecutions: cada passo tem o próprio status."],
      ["aws ssm get-automation-execution --automation-execution-id <automacao-id>"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "get-automation-execution")),
    d("ssmc-fix-auto", "ssm", 3, 130, "Desligue a máquina de teste fora do horário",
      "A máquina de teste fica ligada à noite à toa. Dispare o runbook <b>AWS-StopEC2Instance</b> nela, confira na lista de automações e abra o detalhe pra ver os passos.",
      ["É o mesmo ciclo do reinício, com outro documento.", "Disparar, listar e abrir o detalhe."],
      ["aws ssm start-automation-execution --document-name AWS-StopEC2Instance --parameters InstanceId=<id-da-instância>",
        "aws ssm describe-automation-executions",
        "aws ssm get-automation-execution --automation-execution-id <automacao-id>"],
      (c, cmd, ok) => ok && ehCmd(cmd, "ssm", "get-automation-execution") &&
        automacoes(c).some((a) => a.documento === "AWS-StopEC2Instance")),
  ]);
})();
