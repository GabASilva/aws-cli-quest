...
const DESAFIOS_LINUX = [
  ...
  { id: "lnx-3", servico: "linux", nivel: 1, xp: 40, titulo: "Entre numa pasta",
    ...
    dicas: ["`cd` é de \"change directory\" — trocar de pasta. Você diz pra qual quer ir.", "A forma é: cd <nome-da-pasta>"], solucao: ["cd logs"],
  ...
  { id: "lnx-5", servico: "linux", nivel: 1, xp: 50, titulo: "Crie uma pasta",
    ...
    dicas: ["`mkdir` é de \"make directory\" — criar pasta.", "A forma é: mkdir <nome-da-pasta>"], solucao: ["mkdir backups"],
  ...
  { id: "lnx-6", servico: "linux", nivel: 1, xp: 50, titulo: "Crie um arquivo vazio",
    ...
    dicas: ["`touch` cria um arquivo vazio (e, se já existir, só atualiza a data dele).", "A forma é: touch <nome-do-arquivo>"], solucao: ["touch tarefas.txt"],
  ...
  { id: "lnx-7", servico: "linux", nivel: 2, xp: 70, titulo: "Escreva num arquivo",
    ...
    dicas: ["echo \"<texto>\" > <arquivo> — o `>` joga a saída do echo dentro do arquivo."], solucao: ['echo "Estudar Linux" > tarefas.txt'],
  ...
  { id: "lnx-10", servico: "linux", nivel: 2, xp: 70, titulo: "Renomeie",
    ...
    dicas: ["mv também serve pra renomear: mv <de> <para>"], solucao: ["mv tarefas.txt todo.txt"],
  ...
  { id: "lnx-14", servico: "linux", nivel: 3, xp: 80, titulo: "Conte as linhas",
    ...
    dicas: ["wc -l <arquivo>"], solucao: ["wc -l logs/app.log"],
  ...
  { id: "lnx-18", servico: "linux", nivel: 2, xp: 70, titulo: "Estrutura de uma vez (mkdir -p)",
    ...
    dicas: ["A opção `-p` do mkdir cria a árvore INTEIRA de uma vez — e não reclama se alguma pasta do caminho já existir.", "A forma é: mkdir -p <pasta>/<subpasta>"], solucao: ["mkdir -p deploy/artefatos"],
  ...
  { id: "lnx-19", servico: "linux", nivel: 2, xp: 70, titulo: "Anexe sem apagar (>>)",
    ...
    dicas: ["Com um &gt; só, você SOBRESCREVE. Com &gt;&gt;, você ANEXA.", "A forma é: echo \"<texto>\" &gt;&gt; <arquivo>"],
  ...
  { id: "lnx-20", servico: "linux", nivel: 3, xp: 70, titulo: "Só o começo do log (head)",
    ...
    dicas: ["head -n <quantas-linhas> <arquivo>", "head mostra o começo; tail mostraria o fim."], solucao: ["head -n 3 logs/app.log"],
  ...
  { id: "lnx-21", servico: "linux", nivel: 3, xp: 70, titulo: "Filtre o que interessa (grep)",
    ...
    dicas: ["`grep` filtra linhas que contêm um texto. A ordem importa: primeiro o que procurar, depois onde procurar.", "A forma é: grep <texto> <arquivo>"], solucao: ["grep INFO logs/app.log"],
  ...
  { id: "lnx-22", servico: "linux", nivel: 3, xp: 60, titulo: "Restrinja um arquivo (chmod 600)",
    ...
    dicas: ["chmod <permissão> <arquivo>", "6 = rw- (ler+escrever); 0 = nada."], solucao: ["chmod 600 notas.txt"],
  ...
  { id: "lnx-23", servico: "linux", nivel: 4, xp: 90, titulo: "Prepare o SSH (.ssh em 700)",
    ...
    dicas: ["São dois passos: criar a pasta (mkdir <pasta>) e ajustar a permissão dela (chmod <permissão> <pasta>).", "700 = dono rwx, grupo e outros nada."],
  ...
  { id: "lnx-27", servico: "linux", nivel: 4, xp: 120, titulo: "Projeto do zero (juntando tudo)",
    ...
    dicas: ["São três passos, na ordem: criar a pasta, entrar nela e escrever o arquivo. O `>` joga o texto do `echo` dentro de um arquivo.", "A forma é: mkdir <pasta>  →  cd <pasta>  →  echo \"<texto>\" > <arquivo>"],
