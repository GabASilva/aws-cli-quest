  ...
  const NOVIDADES = [
    {
      ...
      itens: [
        "<b>🔀 Trilha nova: CodeDeploy em Lambda e ECS (blue/green).</b> Em Lambda e em contêiner o CodeDeploy não copia pacote — ele <b>troca o tráfego</b>. São 30 atividades: • no <b>Lambda</b>, o canary manda 10% das chamadas pra versão nova (você vê o <code>RoutingConfig</code> no <code>lambda get-alias</code>) e depois o resto; e um deploy de sexta que é parado com rollback, devolvendo o alias pra versão boa; • no <b>ECS</b>, você monta o serviço com o controlador <code>CODE_DEPLOY</code> atrás de dois target groups, sobe a versão nova no verde, testa enquanto o deploy espera em <b>Ready</b>, libera com <code>continue-deployment</code> e vê o listener trocar de lado no <code>elbv2 describe-listeners</code>. Comandos novos no simulador: <code>lambda get-alias</code>, <code>elbv2 describe-listeners</code> e <code>deploy continue-deployment</code>.",
        "<b>🔌 Trilha nova: CodeConnections — a esteira puxando código do GitHub e do GitLab.</b> A maioria das empresas guarda o código fora da AWS, e é a <b>conexão</b> que deixa a esteira ler esse repositório sem guardar token em lugar nenhum. São 22 atividades: criar a conexão, entender por que ela nasce <b>PENDING</b> (o último passo é humano, no console), ver a esteira falhar com a conexão pendente, registrar o GitHub Enterprise da empresa como <b>host</b> e desligar tudo na ordem que a AWS exige. O comando antigo, <code>aws codestar-connections</code>, também funciona.",
        "<b>🔓 Todas as atividades abertas: faça na ordem que quiser.</b> Nenhuma atividade espera mais a anterior — quer praticar um comando específico ou treinar antes de começar a trilha? Abra direto. Os <b>projetos</b> continuam pedindo as trilhas deles antes, porque as etapas dependem umas das outras. A ordem da trilha continua lá como sugestão: a lista mostra as próximas e esconde o resto numa linha que abre com um clique. E se você pular etapas, o card avisa: algumas atividades usam o que as anteriores criaram. (O limite do plano grátis continua o mesmo.)",
        "<b>📄 CloudFormation: a trilha cresceu de 10 para 44 atividades.</b> Agora é o CloudFormation que você vai usar no trabalho: • um template só pra dev e prod, com <b>Parameters</b>, <code>!Ref</code>, <code>!Sub</code> e <b>Outputs</b> exportados; • <b>rollback</b> de verdade — o create-stack responde sucesso, o stack falha depois, e você acha o motivo em <code>describe-stack-events</code>; • <code>update-stack</code> e <b>change sets</b>, incluindo o que avisa que a tabela seria <b>recriada vazia</b> (e você descarta); • <b>drift</b>: alguém mudou o Lambda na mão e o CloudFormation descobre; • proteção contra exclusão em produção.",
        "<b>⚠️ Uma atividade de CloudFormation ensinava um comando que a AWS recusa.</b> O <code>infra.yaml</code> cria um usuário do IAM com nome próprio, e a AWS só aceita isso com <code>--capabilities CAPABILITY_NAMED_IAM</code> — sem a flag, dá <code>InsufficientCapabilitiesException</code>. A atividade e o exercício do mundo real que usam esse template agora ensinam do jeito certo, e o simulador passou a exigir a flag.",
    ...
    },
    {
      ...
      itens: [
        "<b>🔁 O reforço chegou nas trilhas antigas.</b> Ontem foram os comandos novos; hoje foram os que já estavam no curso e apareciam numa atividade só. S3, EC2, IAM, VPC, EBS, load balancer, Lambda, DynamoDB, RDS, KMS, Secrets Manager, CloudWatch, containers, CloudFront, Route 53 e mais sete serviços ganharam a segunda prática de cada comando, num problema diferente — e depois vieram os serviços menores, de Step Functions a Bedrock. O curso passou de 909 para <b>1.193 atividades</b>, e sobrou só um comando praticado uma vez (criar a organização, que por natureza só se faz uma vez).",
        "<b>🎛️ Escolha quais trilhas aparecem na lista.</b> No topo da lista da esquerda, o botão <b>Escolher trilhas</b> abre todas as trilhas por grupo: marque só o que você está estudando. Tem atalho pra <i>só as grátis</i>, <i>só as que já comecei</i> e por carreira (DevOps, Backend, Dados…). As outras não somem do app — um clique em <i>mostrar todas</i> traz tudo de volta, e trilha nova que entrar aparece sozinha. Se você tem conta, a escolha fica <b>guardada junto com o seu progresso</b>: vale no celular e em qualquer outro computador em que você entrar.",
        "<b>🔎 Busca dentro de cada trilha.</b> Abra uma trilha na lista da esquerda e há um campo de busca no topo: digite <code>delete</code>, <code>--query</code>, <code>tag</code> ou em português mesmo — <i>apagar</i>, <i>parar</i>, <i>criar</i> — e a lista mostra só as atividades daquele assunto. Em cima aparecem os comandos do serviço que casam, com o porquê de cada um; clique pra ir direto à atividade que ensina (ou, se ela ainda está travada, pra mandar o manual pro terminal).",
        "<b>🚚 Grupo novo na lista: Entrega de software (CI/CD)</b> — a família Code, a lacuna mais pedida nas vagas de DevOps: • <b>🗃️ CodeCommit</b>, o repositório Git dentro da conta AWS: 26 atividades e um projeto. Você cria o repositório, faz o primeiro commit sem git instalado (<code>put-file</code>), descobre por que o segundo commit pede o <b>commit pai</b>, abre branch, pede revisão com <b>pull request</b> e mescla por fast-forward — e faz um hotfix do começo ao fim; • <b>🏗️ CodeBuild</b>, o build de CI: 21 atividades e um projeto. Criar a receita do build, rodar, acompanhar fase por fase, ler por que um build <b>falhou</b> (erro de teste) ou deu <b>FAULT</b> (a rede caiu), parar build disparado na branch errada, repetir o que falhou por instabilidade e ajustar máquina e timeout. E um detalhe que confunde muita gente: repetir não conserta código; • <b>🚀 CodeDeploy</b>, a entrega nos servidores: 31 atividades e um projeto. O deploy conversa com o resto do simulador como na AWS: ele acha as máquinas pela <b>tag</b> do EC2 (máquina nova com a tag entra sozinha) e baixa o pacote do <b>S3</b>. Você entrega a v1 e a v2, lê etapa por etapa onde uma máquina quebrou, liga o <b>rollback automático</b> e passa por um simulado de incidente em que a versão nova derruba o health check e volta sozinha.",
        "<b>🔓 Nove atividades não completavam, e agora completam.</b> Desde 17/09, um defeito no jeito como o servidor protege o gabarito deixava o conferidor destas atividades sempre dizendo \"ainda não\", mesmo com o comando certo: <code>piam-u1</code>, <code>piam-u2</code>, <code>piam-u3</code>, <code>piam-r1</code> (IAM), <code>pec2-key1</code> (EC2), <code>plam-1</code> (Lambda), <code>pdyn-1</code>, <code>pdyn-2</code> (DynamoDB) e <code>psqs-url1</code> (SQS). Se você travou numa delas, rode o comando de novo: vai passar. Desculpa pelo susto.",
        "<b>🧾 O IAM agora exige a ordem certa pra desligar alguém.</b> A trilha de IAM ganhou o fim que faltava — apagar usuário, grupo, role e política — e o simulador passou a recusar do jeito da AWS (<code>DeleteConflict</code>): usuário com chave, política ou grupo pendurado não sai; role com política anexada não sai. É exatamente a sequência do desligamento de um funcionário no trabalho de verdade.",
        "<b>⚠️ Cinco atividades ensinavam um comando que a AWS de verdade recusa</b>, e agora estão certas. Conferimos cada uma na documentação oficial: • no <b>DynamoDB</b>, <code>data</code> é palavra reservada, e a query por data precisa do apelido <code>#d</code> com <code>--expression-attribute-names</code>; • no <b>KMS</b>, rotação, habilitar, desabilitar e agendar exclusão só aceitam o <b>KeyId</b>, nunca o alias; • na <b>VPC</b>, uma atividade criava a sub-rede fora da faixa da rede; • e o simulador passou a recusar essas três coisas do mesmo jeito que a AWS recusa, com a mensagem de erro real.",
        ...
        "<b>🎯 O simulador ficou mais parecido com a AWS em dezenas de detalhes</b> que apareceram no caminho: filtros como <code>--filters</code>, <code>--names</code> e prefixos que eram ignorados agora filtram; apagar alarme em lote apagava só o primeiro; publicar versão da Lambda sem deploy no meio agora devolve a mesma versão; target group em uso por um listener não pode mais ser apagado; e o certificado importado pode ser reimportado no mesmo ARN, que é como se renova de verdade. E dois ajustes que você sente no terminal: aspas escapadas (<code>\\\"</code>) dentro de aspas duplas agora funcionam como no shell, e o IAM ganhou o <code>delete-policy-version</code> — política com versão antiga só sai depois de apagar as versões, igual na AWS. E mais dois: <code>file://</code> e <code>fileb://</code> agora acham o arquivo na pasta em que você está (quem dava <code>cd</code> e criava o arquivo lá recebia \"não existe\"), e a dica em português que aparece embaixo do erro parou de mandar conferir ARN de política quando o problema era nome de bucket ou expressão de agendamento. E o <code>--query</code> ficou igual ao da AWS em duas formas muito usadas: <code>Reservations[].Instances[].InstanceId</code> agora devolve a lista de ids (antes saía <code>[null]</code>), e a multisseleção <code>[InstanceId,State.Name]</code> funciona (antes voltava vazia, sem erro).",
      ],
    },
    {
      versao: "2026-09-24",
    ...
    },
    {
      ...
      itens: [
        "<b>🔎 Por que essas e não outras:</b> analisamos vagas de nuvem de verdade e o que quem trabalha na área usa no dia a dia, e fomos engordar primeiro os serviços que aparecem nessas vagas. O curso passou de 690 para <b>810 atividades</b> nesta semana, e cada comando novo entra pelo problema que ele resolve, não pela sintaxe.",
        "<b>🛠️ Operação e suporte — logs e Systems Manager:</b> • <b>log ➜ métrica ➜ alarme</b>, a corrente que substitui alguém encarando a tela de madrugada; • <b>retenção</b>, porque log sem política fica pra sempre e você paga pra sempre; • o <code>tail</code>, o <code>tail -f</code> da nuvem; • <b>Run Command</b>, que roda comando em uma ou trezentas máquinas <i>sem abrir SSH</i>; • <b>Session Manager</b>, terminal sem porta 22 e sem chave .pem; • <b>runbook</b> que o alarme dispara sozinho; e • a resposta do chamado mais comum do suporte: <i>\"não consigo abrir sessão na instância\"</i>.",
        "<b>🔐 Segurança e custo — IAM, EC2 e S3:</b> • <b>rotação de chave</b> na ordem certa (inativar antes de apagar); • o <b>relatório de credenciais</b> que abre toda auditoria; • o <b>simulador de política</b>, que responde \"essa pessoa pode apagar do S3?\" sem testar em produção; • <b>fechar a porta 22</b>, que a trilha ensinava a abrir e nunca a fechar; • <b>IP elástico parado</b>, que é o que a AWS cobra; • as <b>quatro travas</b> contra bucket público; • <b>ciclo de vida</b> do S3, a maior economia que quase ninguém configura; • o <b>presign</b>, link temporário pra arquivo privado; e • o NAT gateway, incluindo apagar ele sem esquecer o IP que continua cobrando. O <code>aws s3</code> virou o terceiro serviço <b>100% coberto</b> do CLImb.",
        "<b>📦 Containers — ECS, ECR e EKS:</b> • <code>run-task</code>, a tarefa avulsa da migração de banco (e a diferença pro service); • <code>execute-command</code>, entrar no contêiner sem SSH; • o <code>stoppedReason</code>, onde mora a resposta de \"por que a tarefa morreu\"; • limpar o registro de imagens, que cresce pra sempre; • escalar o nodegroup — porque pod Pending quase nunca é problema do Kubernetes, é falta de máquina; e • upgrade de cluster, que só sobe uma versão por vez.",
      ],
    },
    {
      versao: "2026-09-21",
      ...
      itens: [
        ...
        "<b>🎯 E o SQS virou o primeiro serviço COMPLETO do CLImb: os 23 comandos que o <code>aws sqs</code> de verdade tem.</b> Entraram os 14 que faltavam, e não são detalhe — são o que separa quem sabe mandar mensagem de quem opera fila em produção: <b>• fila de mensagem morta (DLQ)</b>, pra aquela mensagem quebrada que entra em loop eterno parar de derrubar seu worker de madrugada; <b>• a devolução da DLQ</b>, que reprocessa o que ficou parado depois que você corrigiu o bug — sem script nenhum; <b>• long polling</b>, que corta as requisições vazias que você paga sem perceber; <b>• visibility timeout</b>, que é a causa real daquele pedido cobrado duas vezes; <b>• envio e confirmação em lote</b>, porque a SQS cobra por requisição, não por mensagem; e <b>• etiquetas e permissão entre contas</b>. A trilha foi de 21 pra 40 atividades.",
        "<b>🗺️ Isso é o começo de uma mudança maior.</b> O CLImb vinha cobrindo o recorte do exame CLF-C02; a partir de agora o alvo é o <b>AWS CLI inteiro</b>. Medimos o tamanho real disso rodando o <code>help</code> da CLI de verdade: são <b>4.513 comandos</b> nos serviços que o curso toca, e o CLImb tinha 397. Cada serviço vai ser fechado como o SQS foi, um de cada vez.",
      ],
    },
    {
      versao: "2026-09-15",
    ...
    },
    {
      ...
      itens: [
        "<b>🧗 O ECR virou a primeira trilha com um mini-projeto que atravessa o curso inteiro.</b> Até aqui cada atividade era um exercício solto. Na trilha nova você cria, já na terceira atividade, o repositório <code>pagamentos/checkout-api</code> — e ele te acompanha até o fim: a cada coisa nova que você aprende, você volta e aplica nele. <b>De 5 para 19 atividades</b>, e o serviço deixou de parar no básico:<br>• <b>Varredura de vulnerabilidade</b>: ligar o scan automático e descobrir na prática que ele <b>não olha pra trás</b> — o que já estava guardado continua sem varredura nenhuma. Depois você compara uma imagem de 7 meses com uma de 4 dias e vê a coisa mais desconfortável do assunto: a antiga tem <b>mais falhas sem ninguém ter mexido nela</b>.<br>• <b>Tags travadas</b>: por que “a v2.0” não quer dizer nada enquanto alguém puder publicar por cima dela — e de onde saiu aquela <b>imagem sem nome</b> ocupando espaço no repositório.<br>• <b>Ciclo de vida</b>: a regra que apaga imagem velha pra segurar a conta. Você lê a política com <code>cat</code> antes, <b>ensaia</b> (o preview diz o que sairia, sem apagar nada) e só então aplica — porque o incidente clássico do ECR é a política levar junto a versão que ainda estava rodando em produção.<br>• E o <code>delete-repository</code> agora <b>falha</b> se houver imagem dentro, como falha na AWS de verdade. O erro é a lição.",
        "<b>🧗 Sete trilhas que eram amostra viraram trilha de verdade: 46 atividades novas.</b> ElastiCache tinha <b>3</b> atividades, Polly tinha <b>2</b> — dava pra dizer que o serviço existe, não pra usar no trabalho. Agora cada uma dessas sete tem <b>10 atividades + um projeto final</b> e percorre o ciclo real: observar → criar → usar → configurar → limpar.<br>• <b>ElastiCache</b> (3 → 10) era o caso mais grave: a trilha inteira ensinava a subir um Redis de <b>um nó só</b> — que é exatamente o que ninguém deve pôr em produção — e não contava isso. Agora ela chega no <code>create-replication-group</code> com réplica e <b>failover</b>, e o comando <b>recusa</b> failover com menos de 2 nós, como a AWS faz. Mais subnet group, snapshot e eventos.<br>• <b>Polly</b> (2 → 10): léxico de pronúncia (pra máquina falar o nome da sua marca do jeito certo), narração de texto longo como tarefa assíncrona — e o projeto monta um audiobook.<br>• <b>ACM</b> (4 → 10): onde fica a prova de que o domínio é seu, certificado comprado fora (importação) e <b>o que quebra a renovação automática</b>.<br>• <b>EFS</b> (5 → 10): ciclo de vida — o arquivo esquecido que custa caro —, access point (cada aplicação na sua pasta) e backup, porque <b>no EFS não existe lixeira</b>.<br>• <b>Route 53</b> (6 → 10): por que o site ainda não abre (nameservers), como saber se já propagou e health check pro DNS perceber quando cair.<br>• <b>Secrets Manager</b> (6 → 10): auditar um segredo <b>sem poder ler</b> o conteúdo, parar de inventar senha na mão e fazer a senha trocar sozinha.<br>• <b>CloudFront</b> (5 → 10): conferir se a limpeza terminou, quanto tempo a borda guarda, ETag e o ciclo de vida da CDN — <b>desligar antes de apagar</b>.<br>São <b>676 atividades</b> agora.",
        "<b>🔓 As 3 primeiras atividades de <i>toda</i> trilha agora são abertas.</b> Seis trilhas já eram inteiramente gratuitas; as outras <b>47</b> eram muro — você tinha que decidir assinar sem saber se Lambda, VPC ou Bedrock valiam a pena <i>pra você</i>. Agora dá pra entrar em qualquer uma e experimentar antes. O selo da lateral deixou de ser “🔒 Pro” e virou <b>“3 grátis 🔒”</b>: dizer que está trancada uma trilha em que você <b>pode</b> entrar faz você nem tentar. O projeto final não entra na amostra — ele é o fecho da trilha, não vitrine.",
        "<b>✅ E as páginas de <code>/aprender</code> pararam de prometer o que não cumpriam.</b> As 53 páginas diziam “praticar de graça” — verdade em 6 trilhas, mentira nas outras 47. Agora cada página diz a verdade da <b>sua</b> trilha: “esta trilha é inteiramente gratuita” ou “as 3 primeiras atividades são abertas”. Chegar pelo Google e bater num muro que ninguém avisou é a pior forma de conhecer um site.",
