  ...
  const NOVIDADES = [
    {
      ...
      itens: [
        ...
        "<b>🖥️ E o <code>aws ssm</code> foi de 5 para 19 comandos — com as duas metades que faltavam.</b> Antes o CLImb só ensinava a gaveta de configurações (Parameter Store). Agora tem: <b>• Run Command</b>, que roda um comando em uma ou em trezentas máquinas <i>sem abrir SSH em nenhuma</i>, deixando registrado quem rodou o quê e qual foi a saída; <b>• Session Manager</b>, que abre um terminal na máquina sem porta 22 liberada, sem chave .pem e sem bastion — e audita quem entrou onde; <b>• Automation</b>, o runbook que um alarme dispara sozinho de madrugada, em vez de acordar alguém pro que já se sabe consertar; e <b>• o histórico de versões</b> de um parâmetro, que é como se responde \"quem mudou a config e quebrou a produção\".",
        "<b>🔧 De quebra, a resposta do chamado mais comum do suporte em nuvem:</b> <i>\"não consigo abrir sessão na instância\"</i>. O <code>describe-instance-information</code> mostra quais máquinas o Systems Manager enxerga — e se a sua não está lá, são sempre as mesmas duas causas: o agente não está rodando, ou falta a role com a política certa. A trilha de Systems Manager foi de 7 para 18 atividades.",
        "<b>🔐 E o IAM — o serviço mais pedido em toda vaga de nuvem — foi de 26 para 47 comandos.</b> O CLImb ensinava a criar usuário, grupo, role e política; faltava o que se faz com eles depois: <b>• rotação de chave de acesso</b> na ordem certa (cria a nova, <i>inativa</i> a velha, espera, só então apaga — quem pula a inativação derruba produção, e agora tem uma atividade só pra isso); <b>• o relatório de credenciais</b>, o raio-x que abre toda auditoria; <b>• política inline</b> e a diferença dela pra gerenciada, que cai em prova e em entrevista; <b>• permissions boundary</b>, o teto que deixa você delegar a criação de usuários sem entregar a conta; e <b>• o simulador de política</b>, que responde \"essa pessoa pode apagar do S3?\" <i>sem testar em produção</i> — quase ninguém sabe que ele existe.",
        "<b>🖥️ O EC2 ganhou o que a operação faz com a máquina DEPOIS que ela existe: 39 → 63 comandos.</b> A trilha ensinava a criar, ligar, desligar e apagar. Agora tem: <b>• etiquetas</b>, o comando mais subestimado da AWS — sem elas você fica com 40 máquinas e ninguém sabe de quem é nenhuma; <b>• IP elástico</b>, com a pegadinha que quase todo mundo aprende pagando (IP associado é de graça, o que a AWS cobra é o IP <i>parado</i>); <b>• imagem da máquina</b> antes de mexer em produção, e o motivo de existir o <code>--no-reboot</code>; <b>• rightsizing</b>, trocar o tamanho da instância sem recriar nada; e <b>• fechar a porta</b> — a trilha te ensinava a abrir a 22 e nunca a fechar, que é justamente o que a auditoria cobra.",
        "<b>📦 Containers: ECS, ECR e EKS ganharam 20 comandos de uma vez.</b> A pesquisa de vagas apontou containers como a trilha de melhor retorno — alta demanda e a menor em número de comandos —, então ela veio inteira: <b>• <code>run-task</code></b>, a tarefa avulsa que é como se roda migração de banco e job noturno (e a diferença dela pro service, que confunde todo mundo); <b>• <code>execute-command</code></b>, que é entrar no contêiner rodando <i>sem SSH</i>, com as três condições que fazem ele falhar explicadas no erro; <b>• o diagnóstico</b> — quando a tarefa morre, a resposta está no campo <code>stoppedReason</code>, e quase ninguém sabe olhar aí; <b>• limpeza do registro</b> de imagens, que cresce para sempre e você paga por GB; <b>• escalar o nodegroup</b> do EKS, porque pod Pending quase nunca é problema do Kubernetes: é falta de máquina; e <b>• upgrade de cluster</b>, que só sobe uma versão por vez e é só metade do trabalho.",
        "<b>🪣 O S3 ganhou o andar de baixo — e o <code>aws s3</code> virou o terceiro serviço 100% coberto do CLImb.</b> Entraram os dois que faltavam no alto nível (<code>mv</code> e o <b>presign</b>, que resolve o problema mais comum do S3: entregar um arquivo privado por link temporário <i>sem</i> tornar o bucket público) e mais 16 do <code>s3api</code>: <b>• as quatro travas</b> de bloqueio de acesso público, que são a origem de todo vazamento de S3 que virou notícia; <b>• ciclo de vida</b>, a maior economia de S3 que existe e que quase ninguém configura; <b>• criptografia padrão</b>; <b>• as versões de verdade</b>, onde você vê com os próprios olhos que apagar num bucket versionado <i>não apaga</i> — põe uma lápide e o arquivo continua na fatura; <b>• copiar sem baixar</b>; e <b>• restaurar do Glacier</b>, que leva horas e tem prazo de validade.",
        "<b>🚪 E na trilha de VPC entrou o NAT gateway</b>, que é como a sub-rede privada baixa atualização sem ficar alcançável de fora — e é também uma das linhas mais caras da fatura. As duas atividades ensinam a criar e a <i>apagar</i>, incluindo a parte que quase ninguém lembra: apagar o NAT não libera o IP elástico dele, que continua cobrando sozinho.",
        "<b>🧩 E as trilhas do dia se encaixam:</b> a atividade do <b>instance profile</b> fecha o ciclo com a do Systems Manager. Aquela instância que não aparecia na lista? Máquina EC2 não recebe role direto — recebe um instance profile que carrega a role. É a mesma peça, vista dos dois lados. A trilha de IAM foi de 22 para 37 atividades, e o curso passou de 690 para <b>810</b> nesta semana.",
      ],
    },
    {
      versao: "2026-09-21",
