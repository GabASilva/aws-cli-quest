...
function dadosPublicos(nome, u, pro) {
  ...
  return {
    ...
    // OPT-IN: só é público quem marcou explicitamente. Antes bastava não ter
    // desmarcado (`p.publico !== false`), então toda conta nova nascia visível
    // em /u/<usuario> com XP, atividades, sequência e data de entrada — sem a
    // pessoa ter feito nada. Quando o login vem do Google o usuário é derivado
    // do nome real, o que torna esses dados identificáveis.
...
<!-- sem og:image o perfil compartilhado no LinkedIn virava um retângulo cinza.
     A URL precisa ser ABSOLUTA — crawler não resolve caminho relativo. -->
