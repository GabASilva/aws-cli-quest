...
const ARQUIVOS = [
  ...
  "servicos-fase3.js", "servicos-fase4.js", "servicos-fase5.js",
  ...
  "desafios-extra.js", "desafios-pratica.js", "cloudwatch-metricas.js",
  "logs-insights.js", "lambda-dynamo-profundo.js", "cobertura-1.js",
...
function atividadesDoComando(conteudo, chave) {
  ...
  // Sem regex de propósito: montar padrão com o nome do comando exige escapar
  // hífen e ponto, e um escape a menos passa despercebido (a primeira versão
  // disto virou /^awss+s3s+mb/ e não casava com nada).
  const prefixo = "aws " + servico + " " + sub;
  const casa = (linha) => {
    const t = String(linha).trim();
    return t === prefixo || t.startsWith(prefixo + " ");
  };
  const fora = [];
  for (const d of conteudo.desafios) {
    if (d.tipo === "projeto") continue;
    if ((d.solucao || []).some(casa)) {
...
module.exports = {
  carregar, comandosDaTrilha, atividadesDaTrilha, resumoDoManual, sintaxeDoManual,
